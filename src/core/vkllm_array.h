    ...
    {                                                                                                                  \
        ...
        *arr = (struct vkllm_array_##_name *)malloc(alloc_bytes);                                                      \
        ...
        return VKLLM_ERR_OK;                                                                                           \
                                                                                                                       ...
                                                                                                                       \
    static inline vkllm_err_t vkllm_array_##_name##_copy(struct vkllm_array_##_name *src,                              \
                                                         struct vkllm_array_##_name **dst)                             \
    {                                                                                                                  \
        *dst = (struct vkllm_array_##_name *)malloc(sizeof(*src));                                                     \
        if (*dst == NULL)                                                                                              \
        {                                                                                                              \
            return VKLLM_ERR_ALLOC;                                                                                    \
        }                                                                                                              \
        _NEW_N_AND_CHECK((*dst)->data, _type, src->alloc_n);                                                           \
        (*dst)->alloc_n = src->alloc_n;                                                                                \
        (*dst)->used_n = src->used_n;                                                                                  \
        memcpy((*dst)->data, src->data, sizeof(_type) * src->used_n);                                                  \
        return VKLLM_ERR_OK;                                                                                           \
    }                                                                                                                  \
                                                                                                                       \
