...
vkllm_err_t vkllm_context_new(uint32_t dev, struct vkllm_context **context)
{
    ...
    _CHECK(vkllm_array_pipeline_desc_new(&p->pipelines.vec_add, 32 * sizeof(struct vkllm_pipeline_desc)));
...
void vkllm_context_free(struct vkllm_context *pcontext)
{
    ...
    vkllm_array_pipeline_desc_free(pcontext->pipelines.vec_add);
